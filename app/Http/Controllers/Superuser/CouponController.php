...
class CouponController extends Controller
{
    public function showCouponPage(){
        return view('users.superuser.coupon.super-coupon');
    }
        ...
        return DataTables::of($coupons)
                            ...
                            ->addColumn('typeCus', function($row){
                                if($row->type === 'fixed'){
                                    ...
                                    data-id="'.$row->id.'" 
                                    data-url="'.route('superuser.super.toggle.type.coupon').'"
                                    id="percentCoupon" 
                            ...
                            ->addColumn('actions', function($row){
                                ...
                                <div class="btn-group">
                                    <button type="button" 
                                    data-id="'.$row->id.'" 
                                    data-url="'.route('superuser.super.delete.coupon').'"
                                    id="deleteCoupon" 
                            ...
                            ->addColumn('status', function($row){
                                if($row->status === 'active'){
                                    return ' <button type="button" 
                                    data-id="'.$row->id.'" 
                                    data-url="'.route('superuser.super.toggle.status.coupon').'"
                                    id="deactivateCoupon" 
