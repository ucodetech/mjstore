                  ...
                  <div class="card m-md-auto w-50">
                      ...
                      <div class="card-body">
                        @if ($category->photo != '')
                          ...
                          <hr class="invisible">  
                          <form action="{{ route('seller.vendor.delete.product.category.image') }}" method="post">
                            @csrf
                            @method('POST')  
                          ...
                          @endif
                          <form action="{{ route('seller.vendor.update.product.category') }}" method="POST" enctype="multipart/form-data"
                           >
                           @csrf
                           @method('POST')
                           <input type="hidden" name="category_id" id="category_id" value="{{ $category->id }}">
                                  ...
                                  <select class="custom-select" name="edit_parent_id" id="edit_parent_id">
                                      <option value="">Select Parent Category</option>
                                        @foreach ($p_category as $p_cate)
                                            <option value="{{ $p_cate->id }}" {{ (($category->parent_id == $p_cate->id)? " selected": "") }}>{{ $p_cate->title }} - {{ $p_cate->id }}</option>
                                        @endforeach
                                  </select>
